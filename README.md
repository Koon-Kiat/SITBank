...
```powershell
& "C:\Program Files\PuTTY\pscp.exe" -i "C:\Path\To\your-key.ppk" ".\scamcentre-app.tar.gz" student12@scamcentre.duckdns.org:/tmp/scamcentre-app.tar.gz
```
...
```bash
sudo nginx -T | grep -n "server_name\|proxy_pass\|scamcentre"
```
...
- Self-service account freeze protected by CSRF and MFA; unfreeze is intentionally not exposed.
