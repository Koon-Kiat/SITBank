...
| Logged-in customers (including frozen ones) can submit a categorized support ticket from the account dropdown; bank staff review and triage tickets in a dedicated queue excluding `admin`/`root_admin`; ticket read access uses required (fail-closed) audit and status transitions are an atomic conditional update, so a stale or concurrent transition cannot overwrite a newer decision | `app/web/routes.py::support_contact()`, `support_contact_submit()`; `app/admin/services.py::support_tickets_for_staff()`, `support_ticket_detail_for_staff()`, `transition_support_ticket_status_for_staff()` | `tests/test_support_contact_flow.py`, `tests/test_admin_support_ticket_queue.py` |
