...
- `root_admin`: privileged platform administration, including staff/admin
  invites, invite revocation, direct staff/admin lifecycle changes, manual
  recovery review, and eligible automatic customer-lock unlocks.
...
| Root-admin staff/admin lifecycle actions execute directly after CSRF, root-session, TOTP, no-self-management, target-state, and required-audit checks | `app/admin/services.py::transition_staff_account_as_root_admin()` and `_execute_staff_account_lifecycle()` | `tests/test_admin_maker_checker.py::test_staff_lifecycle_executes_directly_for_root_admin`, `tests/test_admin_dashboard_operations.py::test_root_manages_staff_lifecycle_with_totp_and_safe_audit` |
| Account reset setup only returns a staff/admin account to `setup_pending` and clears MFA; it does not send a setup email, so re-onboarding a reset account uses the separate setup-email resend action | `_execute_staff_account_lifecycle()` reset-activation branch and `resend_staff_setup_invite()` in `app/admin/services.py` | `tests/test_admin_staff_invites.py::test_reset_setup_alone_does_not_resend_email_and_resend_requires_explicit_action` |
| Customer security unlock is root-only, limited to automatic password/MFA lock reasons, TOTP-gated, identity-separated, and direct-executed; execution clears only relevant counters, revokes customer sessions, audits, and notifies | `app/admin/services.py::request_customer_security_unlock()` and `_clear_customer_security_lock()` | `tests/test_admin_maker_checker.py::test_customer_security_unlock_executes_directly_and_clears_only_lock_state`, `tests/test_admin_maker_checker.py::test_customer_security_unlock_fails_closed_for_identity_overlap_and_stale_lock` |
...
Root-admin lifecycle operations use one transaction boundary for the target
mutation and required audit write, so an execution error rolls back the
staff-account change instead of leaving a partial update. Legacy pending
`AdminActionRequest` records remain supported: approval executes before marking
the request `executed`, rejects tampered or stale metadata, and records
...
triage action. Approval, denial, and completion now execute directly for the
root admin after browser CSRF, operator reason, current TOTP, request-state,
self-action, and audit checks pass; completion still requires an already
approved request. Legacy pending approval requests expire, cannot be replayed
after terminal states, and reject tampered metadata.
...
| Staff invite create/revoke/reissue/setup-resend | Root admin session and TOTP code | `app/admin/services.py::create_staff_invite()`, `app/admin/services.py::revoke_staff_invite()`, `app/admin/services.py::reissue_staff_invite()`, `app/admin/services.py::resend_staff_setup_invite()` |
| Staff/admin account lifecycle | Requesting root admin session, CSRF, rate limit, TOTP step-up, no self-management, target-state checks, required audit, direct execution, and safe audit metadata; legacy pending approval records keep HMAC integrity and expiry checks | `app/admin/services.py::transition_staff_account_as_root_admin()`, `app/admin/services.py::approve_admin_action_request_as_root_admin()`, `tests/test_admin_maker_checker.py` |
...
| Manual recovery approval/denial/completion | Requesting root admin session, app-level Flask-WTF browser CSRF enforcement, rate limit, operator reason, TOTP step-up, request-state checks, self-action guard, direct execution, and safe audit metadata; legacy pending approval records keep HMAC integrity and expiry checks | `app/admin/services.py::transition_manual_recovery_request_as_admin()`, `app/admin/services.py::complete_manual_recovery_request_as_admin()`, `app/admin/services.py::approve_admin_action_request_as_root_admin()`, `tests/test_admin_maker_checker.py`, `tests/test_admin_manual_recovery.py` |
