...
- `staff`: bank-staff business-operation responsibility, including the
  ...
  `admin.dispute_transition`), the customer self-freeze unfreeze queue
  (`admin.customer_unfreeze_requests`, `admin.customer_unfreeze`), the
  customer support ticket queue (`admin.support_tickets`,
  `admin.support_ticket_detail`, `admin.support_ticket_transition`), and
  staff-initiated customer account freeze (`admin.customer_freeze_lookup_form`,
  `admin.customer_freeze`), all gated by `require_plain_staff_session` so
  `admin`/`root_admin` sessions are excluded and retain only audit-log
  oversight of those events.
...
| Bank staff can unfreeze a customer's voluntary self-freeze directly (single-approver, no maker-checker) with a required, retained reason and current TOTP step-up; the query uses a positive invariant (`customer`, frozen, and no security lock reason) so automatic and manual/operational lock reasons stay outside this workflow; self-action is blocked, and every write is audited with the reason text | `app/admin/services.py::self_frozen_customers_for_staff()`, `unfreeze_customer_as_staff()`; `admin.customer_unfreeze_requests`/`admin.customer_unfreeze` in `app/admin/routes.py` | `tests/test_admin_self_service_account.py::test_customer_unfreeze_happy_path_records_reason_in_audit`, `test_customer_unfreeze_denied_for_admin_and_root_admin`, `test_customer_unfreeze_rejects_non_self_freeze_lock_target`, `test_customer_unfreeze_blocks_staff_own_linked_customer`, `test_customer_unfreeze_requires_valid_totp_step_up` |
| Logged-in customers (including frozen ones) can submit a categorized support ticket from the account dropdown; bank staff review and triage tickets in a dedicated queue excluding `admin`/`root_admin`; queue rows omit free-text description/resolution detail, detail reads use required (fail-closed) audit, and status transitions are an atomic conditional update, so a stale or concurrent transition cannot overwrite a newer decision | `app/web/routes.py::support_contact()`, `support_contact_submit()`; `app/admin/services.py::support_tickets_for_staff()`, `support_ticket_detail_for_staff()`, `transition_support_ticket_status_for_staff()` | `tests/test_support_contact_flow.py`, `tests/test_admin_support_ticket_queue.py` |
| Bank staff can freeze an active customer's account directly for suspected fraud or a compromised account (single-approver, matching the sibling unfreeze feature) with a required, retained reason and current TOTP step-up; only active, not-already-frozen customer accounts are eligible targets, self-action is blocked, active customer sessions are revoked (`customer_freeze_as_staff` ended reason), every write is audited with the reason text as one durable transaction, and the customer is notified by email only after that transaction commits; the freeze persists a distinct `staff_fraud_freeze` lock reason so it is excluded from the voluntary self-freeze unfreeze queue above and is instead reviewed/resolved through the root-admin customer security-unlock maker-checker workflow | `app/admin/services.py::customer_lookup_for_staff_freeze()`, `freeze_customer_as_staff()`; `admin.customer_freeze_lookup_form`/`admin.customer_freeze` in `app/admin/routes.py` | `tests/test_admin_customer_freeze.py::test_freeze_happy_path_records_reason_and_notifies_customer`, `test_freeze_happy_path_revokes_target_customer_sessions`, `test_freeze_persists_distinct_reason_excluded_from_voluntary_queue`, `test_unfreeze_customer_as_staff_rejects_staff_fraud_frozen_target`, `test_freeze_survives_required_audit_failure`, `test_freeze_denied_for_admin_and_root_admin`, `test_freeze_rejects_already_frozen_target`, `test_freeze_blocks_staff_own_linked_customer`, `test_freeze_requires_valid_totp_step_up` |
| Staff/admin/root-admin self-service password change requires current password, configured password length/policy checks, password-history checks, and current TOTP step-up; success revokes every session for that account (including the one making the change) | `app/admin/services.py::change_own_password()`; `admin.password_change_form`/`admin.password_change_submit` in `app/admin/routes.py` | `tests/test_admin_self_service_account.py::test_password_change_happy_path_forces_relogin_and_audits`, `test_password_change_uses_configured_password_length`, `test_password_change_rejects_wrong_current_password`, `test_password_change_rejects_reused_current_password`, `test_password_change_requires_valid_totp_step_up` |
...
admins without the optional requester reason text; the selected detail view
includes that reason context for reviewer triage, but only after a durable,
fail-closed `manual_recovery_admin_context_read` audit event is recorded —
mirroring the support-ticket detail read-audit contract. If that required
audit write fails, the requester context is not returned. Moving a request to
`under_review` remains a root-admin TOTP-gated triage action. Approval, denial,
and completion create an `AdminActionRequest` that stores only safe metadata
and an HMAC over canonical immutable fields. A different active root admin must
approve with a fresh TOTP code before the centralized
`app/auth/password_reset.py` state machine executes the final change. Pending
approval requests expire, cannot be replayed after terminal states, and reject
tampered metadata.
