...
| Bank staff can freeze an active customer's account directly for suspected fraud or a compromised account (single-approver, matching the sibling unfreeze feature) with a required, retained reason and current TOTP step-up; only active, not-already-frozen customer accounts are eligible targets, self-action is blocked, every write is audited with the reason text, and the customer is notified by email | `app/admin/services.py::customer_lookup_for_staff_freeze()`, `freeze_customer_as_staff()`; `admin.customer_freeze_lookup_form`/`admin.customer_freeze` in `app/admin/routes.py` | `tests/test_admin_customer_freeze.py::test_freeze_happy_path_records_reason_and_notifies_customer`, `test_freeze_denied_for_admin_and_root_admin`, `test_freeze_rejects_already_frozen_target`, `test_freeze_blocks_staff_own_linked_customer`, `test_freeze_requires_valid_totp_step_up` |
...
includes that reason context for reviewer triage. Moving a request to
