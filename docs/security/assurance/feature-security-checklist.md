...
| Transaction history and dispute reporting | Transaction list/detail and dispute creation re-check party ownership server-side on every request (not just the initial link); only one open/under-review dispute per transaction is allowed, enforced by an app-level check and a DB-level partial unique index; a plain-staff-only guard (`require_plain_staff_session`) reviews disputes in the admin app, excluding `admin`/`root_admin`, whose oversight is limited to the audit-log viewer | `app/web/routes.py`, `app/admin/routes.py`, `app/admin/services.py`, `app/models.py`, `tests/test_transaction_history_idor.py`, `tests/test_transaction_dispute_flow.py`, `tests/test_admin_dispute_queue_role_separation.py` |
