...
| Monthly statement export | Opening/closing balance is derived from the live user balance and completed-transaction history rather than trusting a cached view; CSV export neutralizes leading formula-trigger characters and is rate-limited and audited per download | `app/banking/services.py`, `app/banking/routes.py`, `tests/test_monthly_statement.py`, `tests/test_statement_download_security.py` |
