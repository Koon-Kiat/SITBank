...
| CI/CD compromise | Source, workflows, packages, artifacts, protected Tailscale credential | Malicious PR/dependency/runner | Unpinned action, dependency confusion, feature branch secrets, committed credential, exposed scanner session cookie, or abuse of temporary tailnet access | Pinned actions/images, dependency review, custom secret scanner, checksum-verified Gitleaks 8.30.1 full Git history scan with redacted output, trusted-main deployment; private-admin verification is direct protected-environment jobs, no-checkout, explicitly OAuth/auth-key selected, ephemeral `tag:github-ci`, and logs out; host provisioning never runs in CI | `tests/test_deployment.py`, `tests/test_secret_scanner.py`, `tests/test_gitleaks_workflow.py`, `tests/test_tailscale_admin_automation.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Environment protection, selected credential rotation, tag/grant lifecycle, live evidence, and response to any real historical leak remain operator-owned | High |
