...
| CI/CD compromise | Source, workflows, packages, artifacts | Malicious PR/dependency/runner | Unpinned action, dependency confusion, feature branch secrets, malicious package, exposed scanner session cookie | Pinned actions/images, dependency review, hash-locked lockfiles, CodeQL/Bandit/Trivy, trusted-main deployment scripts; release/scheduled DAST uses synthetic identities and a restricted ZAP config file so the DAST cookie is not passed as a raw process argument | `tests/test_deployment.py`, `tests/test_secret_scanner.py`, `tests/test_dast_helper_security.py` | Authenticated DAST on ordinary PRs is an open policy tradeoff | High |
