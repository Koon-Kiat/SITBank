...
| CI/CD compromise | Source, workflows, packages, artifacts | Malicious PR/dependency/runner | Unpinned action, dependency confusion, feature branch secrets, malicious package, exposed scanner session cookie | Pinned actions/images, dependency review, hash-locked lockfiles, CodeQL/Bandit/Trivy, reporting-only SonarQube Cloud analysis, trusted-main deployment scripts; release/scheduled DAST uses synthetic identities and a restricted ZAP config file so the DAST cookie is not passed as a raw process argument; governance review requires closed security issues to update stale docs | `tests/test_deployment.py`, `tests/test_secret_scanner.py`, `tests/test_dast_helper_security.py`, `tests/test_security_governance_docs.py`, `tests/test_sonarqube_workflow.py` | Authenticated DAST on ordinary PRs is an open policy tradeoff; fork PR SonarQube upload is skipped because secrets are withheld | High |
