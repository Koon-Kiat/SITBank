...
| Deployment compromise | Signed image digest, EC2 wrappers, secrets | Compromised CI/job/runner or deploy key | Deploy unverified image, stale wrapper, broad sudo, leaked SSH key | Protected main release flow, Cosign signing, wrapper hash checks, restricted sudoers, environment approval | `tests/test_deployment.py`, `.github/workflows/ci-deploy.yml` | OIDC + SSM migration remains future work | High |
| CI/CD compromise | Source, workflows, packages, artifacts | Malicious PR/dependency/runner | Unpinned action, dependency confusion, feature branch secrets, malicious package | Pinned actions/images, dependency review, hash-locked lockfiles, CodeQL/Bandit/Trivy, trusted-main deployment scripts | `tests/test_deployment.py`, `tests/test_secret_scanner.py` | Authenticated DAST on ordinary PRs is an open policy tradeoff | High |
| Public EC2 edge exposure | SSH, Nginx, Gunicorn, PostgreSQL | Internet scanner or targeted attacker | Open SSH, exposed Gunicorn/PostgreSQL, weak TLS, public admin app | Nginx TLS policy, loopback Gunicorn, public admin denial, OpenSSH drop-in template, UFW/security-group runbook | `tests/test_deployment.py`, `tests/test_ec2_ssh_hardening_docs.py` | Live SSH/UFW/security-group hardening requires operator action | High |
| Staging/admin exposure | Staging app, admin app, private access state | Internet user, unapproved operator | Direct origin bypass, public admin route, Tailscale Funnel, disabled origin-pull | Cloudflare Access/AOP runbook, Nginx origin-pull checks, public admin deny, Tailscale-private admin guidance | `tests/test_zero_trust_access_boundary.py`, `tests/test_deployment.py` | Live Cloudflare/Tailscale state needs operator verification | High |
