...
| CI/CD compromise | Source, workflows, packages, artifacts, protected Tailscale credential | Malicious PR/dependency/runner | Unpinned action, dependency confusion, feature branch secrets, malicious package, exposed scanner session cookie, or abuse of temporary tailnet access | Pinned actions/images, dependency review, scanners, trusted-main deployment; private-admin verification is manual/reusable, protected-environment only, no-checkout, explicitly OAuth/auth-key selected, ephemeral `tag:github-ci`, and logs out; host provisioning never runs in CI | `tests/test_deployment.py`, `tests/test_secret_scanner.py`, `tests/test_tailscale_admin_automation.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Environment protection, selected credential rotation, tag/grant lifecycle, and live evidence remain operator-owned | High |
