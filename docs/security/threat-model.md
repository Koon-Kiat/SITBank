...
| Staging/admin exposure | Staging app, admin app, private access state | Internet user, unapproved operator | Direct origin bypass, public admin route, Access configuration drift, broad policy membership, Tailscale Funnel, disabled origin-pull | Cloudflare Access/AOP runbook, protected provider verification, Nginx origin-pull checks, absent public-admin upstream, read-only EC2 Tailscale Serve/Funnel/listener preflight, and required protected post-production private reachability verification | `tests/test_cloudflare_access_automation.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_deployment.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Operators must execute and retain the live EC2 preflight and separately verify Cloudflare policy plus Tailscale ACL/device/operator state | High |
