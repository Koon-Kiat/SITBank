...
| Security misconfiguration | Partially implemented | `config.py`, `app/security/production_guard.py`, `ops/nginx/*` | `tests/test_production_guard.py`, `tests/test_deployment.py` | Production fail-closed guard, Docker hardening, Nginx TLS and loopback app binding controls | EC2 SSH/UFW/security-group hardening is deferred and not repo-implemented |
...
| Logging and monitoring failures | Implemented | `docs/security/audit-and-alerting.md`, `docs/security/incident-response.md`, `app/security/alerts.py`, `app/admin/routes.py` | `tests/test_audit_alerting.py`, `tests/test_privacy_pdpa_docs.py`, `tests/test_admin_dashboard_operations.py` | Audit hash chain, alert scheduler, sanitized delivery, incident workflows, and admin/root audit review UI | Keep live response evidence outside public issues |
...
| Operations | Partially implemented | `docs/OPERATIONS.md`, `docs/security/security-gap-register.md` | `tests/test_deployment.py` | Alert timer, backup tooling, deployment operations notes, and deferred SSH hardening gap are documented | Live EC2 SSH/UFW and zero-trust state need separate operator validation |
...
| Secure configuration | Partially implemented | `config.py`, `ops/nginx/*`, `ops/deploy/verify-certbot-host-state`, `docs/security/security-gap-register.md` | `tests/test_config.py`, `tests/test_deployment.py` | Production guard, pinned Nginx TLS policy, fail-closed local certificate checks, and deferred EC2 SSH hardening gap are documented | Host SSH/firewall/security-group hardening remains deferred |
| Audit log management | Implemented | `app/security/audit.py`, `app/security/alerts.py`, `app/admin/routes.py` | `tests/test_audit_alerting.py`, `tests/test_admin_dashboard_operations.py` | HMAC chain, append-only triggers, alerts, admin/root audit viewer, and safe alert review | Keep live audit-review evidence outside public issues |
