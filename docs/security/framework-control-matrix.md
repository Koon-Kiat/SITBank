...
| Security misconfiguration | Partially implemented | `config.py`, `app/security/production_guard.py`, `ops/nginx/*`, `docs/security/admin-and-staging-zero-trust-access.md` | `tests/test_production_guard.py`, `tests/test_deployment.py`, `tests/test_zero_trust_access_boundary.py` | Production fail-closed guard, Docker hardening, Nginx TLS, loopback app binding, Cloudflare-managed staging hostname, and private Tailscale admin boundary controls | EC2 SSH/UFW/security-group hardening is deferred; live Cloudflare/Tailscale evidence remains operator-owned |
...
| Produce well-secured software | Implemented | `app/`, `config.py`, `ops/security/*`, `.github/workflows/sonarqube.yml`, `scripts/ci-local` | Full pytest, CodeQL, Bandit, SonarQube, dependency tests, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py` | Secure coding controls, route inventories, production guard, secret scanner, strict local Docker/Compose mode, and reporting-only coverage and maintainability dashboard | Continue adding tests for new features; SonarQube gate remains non-blocking during baseline triage |
...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `scripts/ci-local`, `ops/container/smoke-test.sh` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py` | Full pytest, coverage, reporting-only SonarQube maintainability/duplication analysis, DAST on release/schedule, smoke tests, restricted ZAP `-configfile` cookie handling, and strict Docker/Compose local mode | Authenticated DAST on ordinary PRs remains a policy tradeoff; SonarQube blocking is deferred until baseline triage |
| Operations | Partially implemented | `docs/OPERATIONS.md`, `ops/cloudflare/provision-staging-access`, `app/security/cloudflare_access.py`, `docs/security/security-gap-register.md` | `tests/test_deployment.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_zero_trust_access_boundary.py` | Alert timer, backup tooling, deployment notes, provider verification, origin-side Access assertion enforcement, and private admin Tailscale runbook exist | Live EC2 SSH/UFW, IdP, Tailscale, and cloud state still need operator evidence; zero-trust work is tracked by #198, #199, #200, #210, #211, #215, and #218 |
...
| Vulnerability management | Implemented | `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `.github/dependabot.yml`, `scripts/ci-local` | `tests/test_deployment.py`, `tests/test_sonarqube_workflow.py` | Dependency scans, image scans, CodeQL, Bandit, reporting-only SonarQube dashboard, Dependabot policy | Keep scanner scope and SonarQube baseline under review |
...
| Control area | Status | Relevant files | Relevant tests | Current evidence | Remaining gaps / follow-up |
| --- | --- | --- | --- | --- | --- |
| Access control and MFA | Partially implemented | `docs/security/admin-and-staging-zero-trust-access.md`, `ops/cloudflare/provision-staging-access`, `ops/deploy/verify-cloudflare-origin-pull-ca`, `app/security/cloudflare_access.py`, `app/admin/services.py` | `tests/test_admin_staff_invites.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_cloudflare_origin_pull_ca.py`, `tests/test_zero_trust_access_boundary.py` | Admin TOTP, staff invite lifecycle, fail-closed Cloudflare provider automation, pinned AOP trust, origin assertion validation, and private Tailscale admin model | Live IdP and Tailscale membership require operator verification; tracking references are #198, #199, #200, #210, #211, #215, and #218 |
