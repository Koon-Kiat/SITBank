...
| Produce well-secured software | Implemented | `app/`, `config.py`, `ops/security/*`, `scripts/ci-local` | Full pytest, CodeQL, Bandit, dependency tests, `tests/test_ci_local.py` | Secure coding controls, route inventories, production guard, secret scanner, and strict local Docker/Compose mode for deployment-impacting checks | Continue adding tests for new features |
...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `scripts/ci-local`, `ops/container/smoke-test.sh` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py` | Full pytest, DAST on release/schedule, smoke tests, restricted ZAP `-configfile` cookie handling, and strict Docker/Compose local mode | Authenticated DAST on ordinary PRs remains a policy tradeoff |
