...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `scripts/ci-local`, `ops/container/smoke-test.sh` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py`, `tests/test_sonarqube_pr_comment_workflow.py` | Full pytest, coverage, reporting-only SonarQube maintainability/duplication analysis and sticky internal-PR summary, DAST on release/schedule, smoke tests, restricted ZAP `-configfile` cookie handling, and strict Docker/Compose local mode | Authenticated DAST on ordinary PRs remains a policy tradeoff; SonarQube blocking is deferred until baseline triage; fork and Dependabot PRs intentionally skip secret-backed SonarQube analysis/comments |
