...
| Cryptography and secrets | Implemented | `app/security/crypto.py`, `app/security/session_hmac.py`, `ops/nginx/sitbank-tls-policy.conf`, `ops/deploy/verify-certbot-host-state` | `tests/test_mfa_envelope_crypto.py`, `tests/test_config.py`, `tests/test_deployment.py` | MFA envelope encryption, session/audit HMAC, pinned host TLS policy, local certificate expiry/SAN validation, root-managed secret files | Live TLS evidence remains deployed-endpoint verification |
| Logging and monitoring | Implemented | `app/security/audit.py`, `app/security/alerts.py`, `app/ops/commands.py`, `app/admin/routes.py` | `tests/test_audit_alerting.py`, `tests/test_audit_metadata_sanitization.py`, `tests/test_admin_dashboard_operations.py` | Sanitized audit rows, HMAC hash chain, alert dedupe and delivery sanitization, admin/root audit viewer, and read-only alert review UI | Keep live operational evidence outside public issues |
...
| Security misconfiguration | Partially implemented | `config.py`, `app/security/production_guard.py`, `ops/nginx/*`, `ops/deploy/verify-tailscale-admin-access`, `.github/workflows/tailscale-private-admin-verify.yml`, `docs/security/admin-and-staging-zero-trust-access.md` | `tests/test_production_guard.py`, `tests/test_deployment.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Production fail-closed guard, Docker hardening, Nginx TLS, loopback app binding, Cloudflare-managed staging hostname, private Tailscale admin boundary controls, EC2-local Serve/Funnel/listener verification, and protected reachability verification | EC2 SSH/UFW/security-group hardening is deferred; live Cloudflare policy and Tailscale ACL/device/operator state remain operator-owned |
...
| Logging and monitoring failures | Partially implemented | `docs/security/audit-and-alerting.md`, `docs/security/incident-response.md`, `app/security/alerts.py`, `app/admin/routes.py` | `tests/test_audit_alerting.py`, `tests/test_privacy_pdpa_docs.py`, `tests/test_admin_dashboard_operations.py` | Audit hash chain, alert scheduler, sanitized delivery, incident workflows, admin/root audit review UI, and read-only alert review route | Dedicated audit-viewer hardening remains open; keep live response evidence outside public records |
...
| Produce well-secured software | Implemented | `app/`, `config.py`, `ops/security/*`, `.github/workflows/sonarqube.yml`, `scripts/ci-local` | Full pytest, CodeQL, Bandit, SonarQube, dependency tests, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py`, `tests/test_sonarqube_pr_comment_workflow.py` | Secure coding controls, route inventories, production guard, secret scanner, strict local Docker/Compose mode, reporting-only coverage and maintainability dashboard, and one sticky informational PR summary for trusted internal PRs | Continue adding tests for new features; SonarQube gate remains non-blocking during baseline triage |
...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `ops/deploy/verify-tailscale-admin-access`, `scripts/ci-local`, `ops/container/smoke-test.sh` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py`, `tests/test_sonarqube_pr_comment_workflow.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Full pytest, coverage, reporting-only SonarQube analysis, DAST on release/schedule, smoke tests, strict Docker/Compose local mode, EC2-local Tailscale Serve/Funnel/listener preflight, and protected private-tailnet reachability verification | Authenticated DAST on ordinary PRs remains a policy tradeoff; SonarQube blocking is deferred; protected environment/OAuth-client setup and live tailnet policy/device evidence remain operator-owned |
| Operations | Partially implemented | `docs/OPERATIONS.md`, `ops/cloudflare/provision-staging-access`, `ops/deploy/verify-tailscale-admin-access`, `app/security/cloudflare_access.py`, `.github/workflows/ci-deploy.yml`, `.github/workflows/cloudflare-access-verify.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `docs/security/security-gap-register.md` | `tests/test_deployment.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Alert timer, backup tooling, deployment notes, protected provider verification with explicit environment mapping and redacted field-level drift diagnostics, origin-side Access assertion enforcement, private admin Tailscale runbook, EC2-local host preflight, and required protected post-production reachability gate exist | Live EC2 SSH/UFW, IdP, Tailscale ACL/device/operator membership, and cloud state still need operator evidence |
...
| Audit log management | Partially implemented | `app/security/audit.py`, `app/security/alerts.py`, `app/admin/routes.py`, `app/admin/services.py` | `tests/test_audit_alerting.py`, `tests/test_admin_dashboard_operations.py` | HMAC chain, append-only triggers, alerts, admin/root audit viewer, and safe alert review | Dedicated audit-viewer hardening remains open; keep live audit-review evidence outside public records |
...
| Control area | Status | Relevant files | Relevant tests | Current evidence | Remaining gaps / follow-up |
| --- | --- | --- | --- | --- | --- |
| Access control and MFA | Partially implemented | `docs/security/admin-and-staging-zero-trust-access.md`, `.github/workflows/ci-deploy.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `ops/cloudflare/provision-staging-access`, `ops/deploy/verify-cloudflare-origin-pull-ca`, `ops/deploy/verify-tailscale-admin-access`, `app/security/cloudflare_access.py`, `app/admin/services.py` | `tests/test_admin_staff_invites.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_cloudflare_origin_pull_ca.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Admin TOTP, staff invite lifecycle, fail-closed Cloudflare automation, pinned AOP trust, origin assertion validation, private Tailscale admin model, EC2-local host posture checks, and protected reachability evidence | Live IdP membership and Tailscale membership/ACL/device state require operator verification |
