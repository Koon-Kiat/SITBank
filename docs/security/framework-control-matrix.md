...
| Secure configuration | Partially implemented | `config.py`, `ops/nginx/*`, `docs/security/security-gap-register.md` | `tests/test_config.py`, `tests/test_deployment.py` | Production guard, Nginx TLS policy, and deferred EC2 SSH hardening gap are documented | Host SSH/firewall/security-group hardening remains deferred |
