...
| Security misconfiguration | Partially implemented | `config.py`, `app/security/production_guard.py`, `ops/nginx/*`, `.github/workflows/tailscale-private-admin-verify.yml`, `docs/security/admin-and-staging-zero-trust-access.md` | `tests/test_production_guard.py`, `tests/test_deployment.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Production fail-closed guard, Docker hardening, Nginx TLS, loopback app binding, Cloudflare-managed staging hostname, private Tailscale admin boundary controls, and protected Option B reachability verification | EC2 SSH/UFW/security-group hardening is deferred; host-side live Cloudflare/Tailscale state remains operator-owned |
...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `scripts/ci-local`, `ops/container/smoke-test.sh` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py`, `tests/test_sonarqube_pr_comment_workflow.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Full pytest, coverage, reporting-only SonarQube analysis, DAST on release/schedule, smoke tests, strict Docker/Compose local mode, and manual protected private-tailnet reachability/public-denial verification | Authenticated DAST on ordinary PRs remains a policy tradeoff; SonarQube blocking is deferred; protected environment/key setup and live tailnet evidence remain operator-owned |
| Operations | Partially implemented | `docs/OPERATIONS.md`, `ops/cloudflare/provision-staging-access`, `app/security/cloudflare_access.py`, `.github/workflows/tailscale-private-admin-verify.yml`, `docs/security/security-gap-register.md` | `tests/test_deployment.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Alert timer, backup tooling, deployment notes, provider verification, origin-side Access assertion enforcement, private admin Tailscale runbook, and protected CI reachability verification exist | Live EC2 SSH/UFW, IdP, host-side Tailscale ACL/device/Serve, and cloud state still need operator evidence; zero-trust work is tracked by #198, #199, #200, #210, #211, #215, and #218 |
...
| Session management | Partially implemented | `app/security/sessions.py`, `config.py` | `tests/test_session_management.py`, `tests/test_session_absolute_lifetime.py`, `tests/test_session_risk_binding.py` | Idle timeout, absolute lifetime, secure cookies, customer reauth/revocation and strict admin revocation on risk drift | Active-session cap remains needs-triage; cryptographic device-bound proof is an accepted defense-in-depth item, with admin private boundary decision tracked by #218 |

## MAS TRM / MAS Cyber Hygiene

| Control area | Status | Relevant files | Relevant tests | Current evidence | Remaining gaps / follow-up |
| --- | --- | --- | --- | --- | --- |
| Access control and MFA | Partially implemented | `docs/security/admin-and-staging-zero-trust-access.md`, `.github/workflows/tailscale-private-admin-verify.yml`, `ops/cloudflare/provision-staging-access`, `ops/deploy/verify-cloudflare-origin-pull-ca`, `app/security/cloudflare_access.py`, `app/admin/services.py` | `tests/test_admin_staff_invites.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_cloudflare_origin_pull_ca.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Admin TOTP, staff invite lifecycle, fail-closed Cloudflare automation, pinned AOP trust, origin assertion validation, private Tailscale admin model, and manual protected CI reachability evidence | Live IdP membership and host-side Tailscale membership/ACL/device/Serve state require operator verification; tracking references are #198, #199, #200, #210, #211, #215, and #218 |
