...
| Verification | Partially implemented | `tests/`, `.github/workflows/ci-deploy.yml`, `.github/workflows/sonarqube.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `ops/tailscale/*`, `ops/deploy/verify-tailscale-admin-access`, `scripts/ci-local` | `tests/test_deployment.py`, `tests/test_dast_helper_security.py`, `tests/test_ci_local.py`, `tests/test_sonarqube_workflow.py`, `tests/test_tailscale_admin_access.py`, `tests/test_tailscale_admin_automation.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Full pytest, reporting-only SonarQube, DAST, smoke tests, strict local mode, confirmation-gated Tailscale automation contracts, EC2-local preflight, and protected private-tailnet reachability verification | Authenticated DAST on ordinary PRs remains a policy tradeoff; live Tailscale policy/device evidence remains operator-owned |
