...
| Control area | Status | Relevant files | Relevant tests | Current evidence | Remaining gaps / follow-up |
| --- | --- | --- | --- | --- | --- |
| Access control and MFA | Partially implemented | `docs/security/admin-and-staging-zero-trust-access.md`, `.github/workflows/ci-deploy.yml`, `.github/workflows/tailscale-private-admin-verify.yml`, `ops/cloudflare/provision-staging-access`, `ops/deploy/verify-cloudflare-origin-pull-ca`, `app/security/cloudflare_access.py`, `app/admin/services.py` | `tests/test_admin_staff_invites.py`, `tests/test_cloudflare_access_automation.py`, `tests/test_cloudflare_access_staging.py`, `tests/test_cloudflare_origin_pull_ca.py`, `tests/test_zero_trust_access_boundary.py`, `tests/test_tailscale_ci_tailnet_workflow.py` | Admin TOTP, staff invite lifecycle, fail-closed Cloudflare automation, pinned AOP trust, origin assertion validation, private Tailscale admin model, and protected reachability evidence on demand and after production public TLS | Live IdP membership and host-side Tailscale membership/ACL/device/Serve state require operator verification |
